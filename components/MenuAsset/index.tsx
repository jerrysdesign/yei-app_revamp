...
import { assets } from "@/constants/navigation";
...
const MenuAsset = ({ className, buttonIcon }: MenuAssetProps) => {
    ...
    return (
        <div className={className}>
            <Menu>
                {buttonIcon ? (
                    <Menu.Button className="flex justify-center items-center w-100 h-100"> 
                        <Icon
                    ...
                    <Menu.Button className="flex items-center max-w-full text-left text-h6 transition-colors md:text-body-1 hover:text-primary-3 tap-highlight-color">
                        {assets
                            ...
                            .map((item) => (
                                ...
                                >
                                    <div
                                        className="absolute top-1/2 left-0 -translate-y-1/2 flex justify-center items-center shrink-0 w-10 h-10 mr-5 rounded-full md:hidden"
                    ...
                    <Menu.Items className="absolute top-full left-3 right-3 -mt-1 p-3 bg-n-1 rounded-2xl shadow-modal outline-none dark:bg-n-7">
                        <div className="max-h-[24rem] overflow-auto scroll-smooth max-h-850:max-h-[18rem] md:max-h-[24rem]">
                            {assets.map((item: any) => (
                                ...
                                >
                                    <div
                                        className="flex justify-center items-center w-6 h-6 mr-7 rounded-full md:mr-4"
                                        style={{ backgroundColor: item.color }}
                                    >
                                        <Image
                                            className="w-8"
