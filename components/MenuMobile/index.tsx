...
const MenuMobile = ({ className, buttonIcon }: MenuMobileProps) => {
                        ...
                        {mainMenuList
                            ...
                            .map((item) => (
                                <div
                                    ...
                                    key={item.id}
                                >
                                    <div className="absolute top-1/2 left-0 -translate-y-1/2 flex justify-center items-center shrink-0 w-10 h-10 mr-5 rounded-xl md:hidden">
                                        <Image
                                            className="w-full"
                                            src={item.image}
                                            width={24}
                        ...
                        <div className="max-h-[24rem] overflow-auto scroll-smooth max-h-850:max-h-[18rem] md:max-h-[24rem]">
                            {mainMenuList.map((item: any) => (
                                <Menu.Item
                                    ...
                                    href={item.url}
                                >
                                    <div className="flex justify-center items-center w-6 h-6 mr-7 md:mr-4">
                                        <Image
                                            className="w-full"
                                            src={item.image}
                                            width={24}
